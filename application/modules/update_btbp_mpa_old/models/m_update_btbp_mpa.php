...
class M_update_btbp_mpa extends CI_Model {
	...
	{
		$sql = "SELECT ORIGIN, CITY_ZONE_CODE, CITY_ZONE_3CODE, CITY_ZONE_KABKOTA,CITY_ZONE_KECAMATAN, CITY_ZONE_ID,LINEHAUL, LINEHAUL_NEXT, TRANSIT, DL_BEFORE_OKE,DL_BEFORE_REG, DL_BEFORE_YES, DL_BEFORE_SPS, NVL(DL_BEFORE_NEXT_SPS,0) DL_BEFORE_NEXT_SPS, DROURATE_SERVICE FROM (SELECT SUBSTR(DROURATE_CODE,1,8) ORIGIN, CITY_ZONE_CODE, CITY_ZONE_3CODE, CITY_ZONE_KABKOTA, CITY_ZONE_KECAMATAN, CITY_ZONE_ID,DROURATE_SERVICE, nvl(DROURATE_LINEHAUL,0) LINEHAUL, nvl(DROURATE_LINEHAUL_NEXT,0) LINEHAUL_NEXT,nvl(DROURATE_TRANSIT,0) TRANSIT, CASE WHEN DROURATE_SERVICE LIKE 'OKE%' THEN DROURATE_DELIVERY ELSE 0 END DL_BEFORE_OKE, CASE WHEN DROURATE_SERVICE LIKE 'REG%' THEN DROURATE_DELIVERY ELSE 0 END DL_BEFORE_REG, CASE WHEN DROURATE_SERVICE LIKE 'YES%' THEN DROURATE_DELIVERY ELSE 0 END DL_BEFORE_YES, CASE WHEN DROURATE_SERVICE LIKE 'SPS%' THEN DROURATE_DELIVERY ELSE 0 END DL_BEFORE_SPS, CASE WHEN DROURATE_SERVICE LIKE 'SPS%' THEN (SELECT DISTINCT DZONE_PRICE FROM CMS_DZONE WHERE DROURATE_ZONE=DZONE_CODE AND DROURATE_SERVICE LIKE 'SPS15' AND DZONE_SERV = DROURATE_SERVICE AND DZONE_TYPE='1' AND DZONE_LEVEL=2) ELSE 0 END DL_BEFORE_NEXT_SPS 
			FROM V_TARIF_UTCU 
			WHERE
			SUBSTR(DROURATE_CODE,9,6)= SUBSTR('$cabang',1,6) 
			AND CITY_ZONE_CODE = SUBSTR(DROURATE_CODE,9,8) 
			AND DZONE_LEVEL = '1' AND DZONE_TYPE='1' AND DROURATE_SERVICE IN (SELECT SERVICE_CODE FROM CMS_SERVICE WHERE SERVICE_CODE LIKE 'OKE%' OR SERVICE_CODE LIKE 'REG%' OR SERVICE_CODE LIKE 'YES%' OR SERVICE_CODE LIKE 'SPS%') AND DROURATE_ACTIVE='Y' AND DROURATE_DELIVERY>0) GROUP BY ORIGIN, CITY_ZONE_CODE, CITY_ZONE_3CODE, CITY_ZONE_KABKOTA,CITY_ZONE_KECAMATAN, LINEHAUL, LINEHAUL_NEXT, TRANSIT, CITY_ZONE_ID, DL_BEFORE_OKE,DL_BEFORE_REG, DL_BEFORE_YES, DL_BEFORE_SPS, DL_BEFORE_NEXT_SPS, DROURATE_SERVICE";
		$query = $this->db->GetArray($sql);
		return $query;
	}
									  ...
									  $user_origin, $user_zone_code, $user_level, $session_request, $regional)
	{
		...
		$sql = "INSERT INTO TARIF_MASTER_BTBP
				(NO_REQUEST, DESTINATION, USER_ID, USER_NAME, USER_BRANCH, USER_ORIGIN, USER_ZONE_CODE, USER_LEVEL,
	...
	{
		$sql = "SELECT NO_REQUEST FROM TARIF_MASTER_BTBP WHERE SESSION_REQUEST = '$session'";
		$query = $this->db->GetArray($sql);
		return $query;
	}
	...
	{
		$sql = "SELECT NO_REQUEST, ORIGIN, USER_ID FROM TARIF_MASTER_BTBP WHERE SESSION_REQUEST = '$session'";
		$query = $this->db->GetArray($sql);
		return $query;
	}
	...
	{
		$sql = "SELECT * FROM TARIF_DETAIL_BTBP WHERE NO_REQUEST = '$no_request'";
		$query = $this->db->GetArray($sql);
		return $query;
	}

	public function save_btbp_level_2($no_request, $status, $notice, $user_id)
	{
		$sql = "UPDATE TARIF_MASTER_BTBP SET STATUS_REGIONAL = '$status', PIC_REGIONAL = '$user_id',
	...
	public function level_3()
	{
		$sql = "SELECT CREATED, NO_REQUEST, ORIGIN, SESSION_REQUEST, STATUS_MPA,
				STATUS_TESTING, TGL_UPDATE_BTBP FROM TARIF_MASTER_BTBP
	...
	public function notice_approve_1($session)
	{
		$sql = "SELECT A.PIC_REGIONAL, A.UPDATE_STATUS_REGIONAL, B.STATUS_REQUEST, A.NOTICE_REGIONAL
				...
				WHERE A.SESSION_REQUEST = '$session'";
		$query = $this->db->GetArray($sql);
		return $query;
	}

	public function save_btbp_level_3($no_request,
								  $status, $user_id, $notice, $tgl_update_btbp)
	{
		$sql = "UPDATE TARIF_MASTER_BTBP SET STATUS_MPA = '$status', PIC_MPA = '$user_id',
	...
	{
		$sql = "SELECT STATUS_MPA FROM TARIF_MASTER_BTBP WHERE NO_REQUEST = '$no_request'";
		$query = $this->db->GetArray($sql);
		return $query;
	}
	...
	{
		$sql = "SELECT STATUS_TESTING FROM TARIF_MASTER_BTBP WHERE NO_REQUEST = '$no_request'";
		$query = $this->db->GetArray($sql);
		return $query;
	}
	...
	public function save_testing_level_3($no_request, $status, $user_id, $notice, $tgl_live_btbp, $tgl_close)
	{
		$sql = "UPDATE TARIF_MASTER_BTBP SET STATUS_TESTING = '$status', TGL_LIVE_BTBP = '$tgl_live_btbp',
				PIC_LIVE = '$user_id', NOTICE_LIVE = '$notice', TGL_APPROVE_TESTING = SYSDATE,
				TGL_CLOSING = '$tgl_close' WHERE NO_REQUEST = '$no_request'";
		$query = $this->db->Execute($sql);
		return $query;
	}
