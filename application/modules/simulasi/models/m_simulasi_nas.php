...
class M_simulasi_nas extends CI_Model {
	...
	public function get_branch_nas_after($branch)
	{
		$sql = "SELECT BRANCH_CODE, BRANCH_DESC FROM ORA_BRANCH WHERE BRANCH_REGION <>'I'
				AND (BRANCH_CODE LIKE '$branch%' OR BRANCH_DESC LIKE '$branch%')";
		$query = $this->db->GetArray($sql);
	...
	public function data_branch_nas_after($branch)
	{
		$sql = "SELECT BRANCH_CODE, BRANCH_DESC FROM ORA_BRANCH WHERE BRANCH_REGION <>'I'
				AND (BRANCH_CODE LIKE '$branch%' OR BRANCH_DESC LIKE '$branch%')";
		$query = $this->db->GetArray($sql);
	...
	public function get_origin_nas_after($branch,$origin)
	{
	...
	public function data_origin_nas_after($branch,$origin)
	{
		$sql = "SELECT CITY_CODE,CITY_NAME FROM CMS_CITY, TARIF_CMS_DROURATE WHERE CITY_MAIN_BRANCH = SUBSTR('$branch',1,6) AND CITY_ACTIVE = 'Y'
                AND CITY_CODE=SUBSTR(DROURATE_CODE,1,8)
                AND DROURATE_ACTIVE='N'
	...
	public function get_destination_nas_after($origin,$destination)
	{
		$sql = "SELECT 
				CITY_CODE,CITY_NAME  
	...
	public function data_destination_nas_after($origin,$destination)
	{
		$sql = "SELECT 
				CITY_CODE,CITY_NAME  
				FROM CMS_CITY, TARIF_CMS_DROURATE WHERE CITY_ACTIVE = 'Y'
	...
	public function get_service_nas_after($params)
	{
		$sql = "SELECT DROURATE_SERVICE FROM TARIF_CMS_DROURATE, CMS_SERVICE 
                WHERE DROURATE_CODE = SUBSTR('$params',1,16) AND DROURATE_ACTIVE='N'
                AND SERVICE_CODE=DROURATE_SERVICE
...
/* End of file m_simulasi_nas.php */
/* Location: ./application/modules/simulasi/models/m_simulasi_nas.php */