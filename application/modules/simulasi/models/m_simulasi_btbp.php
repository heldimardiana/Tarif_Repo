...
class M_simulasi_btbp extends CI_Model {
	...
	public function get_btbp_testing($no_request)
	{
		$sql = "SELECT A.*, ROWID FROM TARIF_CMS_DROURATE A WHERE DROURATE_NO_REQUEST = '$no_request'";
	...
	public function get_origin_a($origin,$no_request)
	{
		$sql = "SELECT CITY_CODE,CITY_NAME FROM CMS_CITY, TARIF_CMS_DROURATE WHERE CITY_ACTIVE = 'Y'
                AND CITY_CODE=SUBSTR(DROURATE_CODE,1,8)
                AND DROURATE_ACTIVE='Y'
                AND (CITY_CODE LIKE '$origin%' ||'%' OR CITY_NAME LIKE '%'||'$origin%'||'%')
	...
	public function data_origin_a($origin,$no_request)
	{
		$sql = "SELECT CITY_CODE,CITY_NAME FROM CMS_CITY, TARIF_CMS_DROURATE WHERE CITY_ACTIVE = 'Y'
                AND CITY_CODE=SUBSTR(DROURATE_CODE,1,8)
                AND DROURATE_ACTIVE='Y'
	...
	public function generate_simulai_btbp_before($origin)
	{
        $sql = "SELECT
	...
	public function generate_simulai_btbp_after($no_request,$origin)
	{
        $sql = "SELECT DROURATE_CODE,CITY_ZONE_CODE,CITY_ZONE_3CODE,CITY_ZONE_KABKOTA,
                CITY_ZONE_KECAMATAN,CITY_ZONE_ID, SERVICE_DISPLAY,
                PENERUS_BEFORE_NEXT,
                PENERUS_BEFORE,PENERUS_NEXT,PENERUS,BT_BEFORE,BT,BD_BEFORE_OKE,
                BD_OKE,BD_BEFORE_REG,BD_REG,BD_BEFORE_YES,BD_YES,BD_BEFORE_SPS,
                BD_SPS,BD_BEFORE_NEXT_SPS,BD_NEXT_SPS 
                FROM V_BTBP_AFTER_BEFORE WHERE DROURATE_NO_REQUEST = '$no_request'
                AND SUBSTR(DROURATE_CODE,1,8) = '$origin'
                GROUP BY 
                DROURATE_CODE,CITY_ZONE_CODE,CITY_ZONE_3CODE,CITY_ZONE_KABKOTA,
                CITY_ZONE_KECAMATAN,CITY_ZONE_ID,SERVICE_DISPLAY, PENERUS_BEFORE_NEXT,
                PENERUS_BEFORE,PENERUS_NEXT,PENERUS,BT_BEFORE,BT,BD_BEFORE_OKE,
                BD_OKE,BD_BEFORE_REG,BD_REG,BD_BEFORE_YES,BD_YES,BD_BEFORE_SPS,
                BD_SPS,BD_BEFORE_NEXT_SPS,BD_NEXT_SPS
                ORDER BY CITY_ZONE_CODE";
...
/* End of file m_simulasi_btbp.php */
/* Location: ./application/modules/simulasi/models/m_simulasi_btbp.php */