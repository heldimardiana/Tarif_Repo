...
class M_simulasi_utc extends CI_Model {
	...
	public function get_cabang_utama_utc_a($cabang_utama)
	{
		$sql 	= "SELECT BRANCH_CODE, BRANCH_CITY, BRANCH_DESC FROM ORA_BRANCH WHERE BRANCH_REGION<>'I' AND 
				   (BRANCH_CODE LIKE '$cabang_utama%' OR BRANCH_DESC LIKE '$cabang_utama%')";
	...
	public function data_cabang_utama_utc_a($cabang_utama)
	{
		$sql 	= "SELECT BRANCH_CODE, BRANCH_CITY, BRANCH_DESC FROM ORA_BRANCH WHERE BRANCH_REGION<>'I' AND 
				   (BRANCH_CODE LIKE '$cabang_utama%' OR BRANCH_DESC LIKE '$cabang_utama%')";
		$query 	= $this->db->GetArray($sql);
	...
	public function get_cabang_utc_a($cabang_utama,$cabang)
	{
	...
	public function data_cabang_utc_a($cabang_utama,$cabang)
	{
		$sql	= "SELECT CITY_CODE,CITY_NAME FROM CMS_CITY, TARIF_CMS_DROURATE WHERE CITY_MAIN_BRANCH = SUBSTR('$cabang_utama',1,6)  
					AND CITY_ACTIVE = 'Y'
					AND CITY_CODE=SUBSTR(DROURATE_CODE,1,8)
	...
	public function get_destination_utc_a($origin,$destination)
	{
		$sql = "SELECT 
				CITY_CODE,CITY_NAME  
	...
	public function data_destination_utc_a($origin,$destination)
	{
		$sql = "SELECT 
				CITY_CODE,CITY_NAME  
				FROM CMS_CITY, TARIF_CMS_DROURATE WHERE CITY_ACTIVE = 'Y'
	...
	public function data_service_utc_a($params)
	{
	...
	public function get_tarif_after($param)
	{
...
/* End of file m_simulasi_utc.php */
/* Location: ./application/modules/simulasi/models/m_simulasi_utc.php */