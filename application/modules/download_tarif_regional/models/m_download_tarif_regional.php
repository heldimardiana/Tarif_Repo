...
class M_download_tarif_regional extends CI_Model {
	...
	public function get_branch($regional,$branch)
	{
		$sql 	= "SELECT BRANCH_CODE, BRANCH_CITY, BRANCH_DESC FROM ORA_BRANCH WHERE (BRANCH_REGION<>'I' AND 
				   BRANCH_OPS_REGION = '$regional') AND (BRANCH_CODE LIKE '%$branch%' OR BRANCH_DESC LIKE '$branch%')";
	...
	public function data_branch($regional,$branch)
	{
		$sql 	= "SELECT BRANCH_CODE, BRANCH_CITY, BRANCH_DESC FROM ORA_BRANCH WHERE (BRANCH_REGION<>'I' AND 
				   BRANCH_OPS_REGION = '$regional') AND (BRANCH_CODE LIKE '%$branch%' OR BRANCH_DESC LIKE '$branch%')";
		$query 	= $this->db->GetArray($sql);
	...
	public function generate_tarif_regional_after($routing,$service,$no_request)
	{
	...
	public function generate_tarif_regional_afters($routing,$service,$no_request)
	{
		$sql = "SELECT * FROM V_TARIF_DETAIL_TESTING_2 WHERE ROUTING = '".$this->db->escape($routing)."' 
				AND DROURATE_SERVICE = '".$this->db->escape($service)."' 
				AND DROURATE_NO_REQUEST = '".$this->db->escape($no_request)."'";
		$query = $this->db->GetArray($sql);
		return $query;
...
/* End of file m_download_tarif_regional.php */
/* Location: ./application/modules/download_tarif_regional/models/m_download_tarif_regional.php */